...
def main():
    ...
    parser.add_argument("--from-branch", help="use a branch of dharitri/drt-sdk-py-cli")
...
""")
    ...
    install_drtpy(exact_version, from_branch, verbose)
...
def guard_non_root_user():
    ...
    if operating_system == "windows":
        return
    if os.getuid() == 0:
        raise InstallError("You should not install drtpy as root.")
...
You may need to reinstall wasm-opt using `drtpy deps install wasm-opt`.
""")
        confirm_continuation(interactive)
...
def create_venv():
    ...
    venv_folder = get_drtpy_venv_path()
...
def require_python_venv_tools():
    ...
    try:
        ...
        if operating_system == "linux":
            ...
            raise InstallError(f'Packages [venv] or [ensurepip] not found. Please run "sudo apt install {python_venv}" and then run drtpy-up again.')
        else:
...
def get_drtpy_venv_path():
    return sdk_path / "drtpy-venv"


def install_drtpy(exact_version: str, from_branch: str, verbose: bool):
    logger.debug("Installing drtpy in virtual environment...")

    if from_branch:
        package_to_install = f"https://github.com/DharitriOne/drt-sdk-py-cli/archive/refs/heads/{from_branch}.zip"
    else:
        package_to_install = "drt_sdk_cli" if not exact_version else f"drt_sdk_cli=={exact_version}"

    venv_path = get_drtpy_venv_path()
    ...
    if return_code != 0:
        raise InstallError("Could not install drtpy.")

    logger.debug("Creating drtpy shortcut...")

    shortcut_path = sdk_path / "drtpy"
    ...
    shortcut_content = get_drtpy_shortcut_content()
...
def get_drtpy_shortcut_content():
    operating_system = get_operating_system()
    venv_path = get_drtpy_venv_path()

    if operating_system == "windows":
        return f"""#!/bin/sh
. "{venv_path / 'Scripts' / 'activate'}" && python3 -m drt_sdk_cli.cli "$@" && deactivate
"""

    return f"""#!/bin/sh
. "{venv_path / 'bin' / 'activate'}" && python3 -m drt_sdk_cli.cli "$@" && deactivate
"""
...
def run_post_install_checks():
    drt_sdk_path = Path("~/drt-sdk").expanduser()

    logger.debug("Running post-install checks...")

    if drt_sdk_path.exists():
        logger.debug("~/drt-sdk exists  OK")
    else:
        logger.warning("~/drt-sdk exists  NOK")

    if (drt_sdk_path / "drtpy").exists():
        logger.debug("~/drt-sdk/drtpy shortcut created  OK")
    else:
        logger.warning("~/drt-sdk/drtpy shortcut created  NOK")
...
def guide_system_path_integration():
    ...
    if operating_system == "windows":
        print(f"""
###############################################################################
On Windows, for the "drtpy" command shortcut to be available, you MUST ADD the directory "{sdk_path}" to the system PATH.
...
""")
    ...
    old_export_directive = f'export PATH="{Path("~/numbatsdk").expanduser()}:$PATH"\t# numbat-sdk'
    new_export_directive = f'export PATH="${{HOME}}/drt-sdk:$PATH"\t# drt-sdk'
...
The "drtpy" command shortcut will not be available until you add the directory "{sdk_path}" to the system PATH.
###############################################################################
Do you UNDERSTAND the above?
""")
        confirm_continuation(interactive)
    ...
    if any_old_export_directive:
        ...
        print(f"""
###############################################################################
It seems that the old path "~/numbatsdk" is still configured in shell profile.
...
""")
    ...
    if any_new_export_directive:
        ...
        print(f"""
###############################################################################
It seems that the path "~/drt-sdk" is already configured in shell profile.
...
""")
    ...
    print(f"""
###############################################################################
In order to use the "drtpy" command shortcut, you have to manually extend the PATH variable to include "~/drt-sdk".

In order to manually extend the PATH variable, add the following line to your shell profile file upon installation:

    export PATH="${{HOME}}/drt-sdk:${{PATH}}"
...
###############################################################################
For more information go to https://docs.dharitri.com.
For support, please contact us at http://discord.gg/DharitriBuilders.
###############################################################################
""")
