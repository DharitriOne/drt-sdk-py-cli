...
from drt_sdk_wallet import Mnemonic, UserPEM, UserWallet
...
def test_wallet_new_as_pem():
    ...
    assert UserPEM.from_file(outfile).label.startswith("moa1")
...
def test_wallet_convert_raw_mnemonic_to_pem():
    ...
    assert pem.label == "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"
...
def test_wallet_bech32_encode(capsys: Any):
    ...
    assert out == "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"


def test_wallet_bech32_decode(capsys: Any):
    main([
        "wallet", "bech32", "--decode", "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"
    ])
...
def test_verify_previously_signed_message(capsys: Any):
    ...
    out = _read_stdout(capsys)
    text = """SUCCESS: The message "test" was signed by moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8""".split()
...
def test_verify_not_signed_message(capsys: Any):
    ...
    out = _read_stdout(capsys)
    text = """FAILED: The message "this message is not signed" was NOT signed by moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8""".split()
    assert all(word in out for word in text)
...
def test_sign_and_verify_message_with_multi_address_pem(capsys: Any):
    ...
    assert False if return_code else True
    assert out == {
        "address": "moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk",
        "message": "test",
    ...
    out = _read_stdout(capsys)
    text = """SUCCESS: The message "test" was signed by moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk""".split()
    assert all(word in out for word in text)
    ...
    out = _read_stdout(capsys)
    text = """SUCCESS: The message "test" was signed by moa1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaqhr5l9h""".split()
    assert all(word in out for word in text)
